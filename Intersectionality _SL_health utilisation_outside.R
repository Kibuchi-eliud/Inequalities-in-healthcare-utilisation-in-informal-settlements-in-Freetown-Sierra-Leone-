...
crosstable::crosstable(data_cleaned, hh_income_private_salaried, by = Health_utilisation_outside, total ="all")
...
#"waste_sea"
crosstable::crosstable(data_cleaned, waste_sea, by = Health_utilisation_outside, total ="all")
...
HU_outside_model_1<- brm(Health_utilisation_outside  ~ 1  + (1|strata_id) + offset(household_number), 
...
write.csv(HU_outside_model_1_exp, "results/HU_outside_model_1_exp.csv")
...
#With an AUC score of  0.66, the model discrimination is poor
#=====================================================================================================
...
variance_group <-   3.025^2; variance_group 
VPC_model1 <-   3.025^2 / (3.025^2 +3.29)*100; VPC_model1
#model1_vpc = 73.55% - indicates high discriminatory accuracy of intersection strata 
...
#Export data
write.csv(HU_outside_model1, file = "results/HU_outside_model1.csv")
...
write.csv(HU_outside_model_2_exp, "results/HU_outside_model_2_exp.csv")
...
model2_variance <- 1.915^2; model2_variance 
model2_vpc <-   1.915^2 / (1.915^2 +3.29)*100; model2_vpc
#52.61%

#PCV
model1_variance <-  3.025^2
model2_variance <-  1.915^2
model2_PCV <- (3.025^2 - 1.915^2)/ 3.025^2 ; model2_PCV
model2_PCV <- model2_PCV*100; model2_PCV
#PCV=60.09%
#This indicates 39.1% of variance was not explained by adding fixed effects 
#=================================================================================================
#Plot
...
#Export data
write.csv(HU_outside_model2, file = "results/HU_outside_model2.csv")
...
HU_outside_model_3 <- brm(Health_utilisation_outside ~ 1  + head_gender + marital_status +
                        income_activity + hh_disabled + food_security + community_type + length_of_stay + hh_income_business +
                          hh_income_govt_salaried + hh_income_private_salaried + hh_income_daily_wage + hh_income_others + hh_tenure +
                          water_cook_piped_neighbor + water_cook_publictap_standpipe +water_cook_compound_well + water_cook_spring + 
                          water_cook_rain + water_cook_bowser + water_cook_kiosk + water_cook_bottled + water_sachet + water_cook_surface + 
                          water_cook_neighor_well + water_cook_other + water_cook_distance + water_shortage + toilet_flush + toilet_bucket + 
                          toilet_latrine + toilet_hanging + toilet_open_defecation + waste_dumping_site + waste_drainage + waste_solid_collectors +
                          waste_sea + waste_others + waste_payment + environ_disaster + (1|strata_id) + offset(household_number), 
...
HU_outside_model_3_exp <- exp(fixef(HU_outside_model_3)); HU_outside_model_3 
write.csv(HU_outside_model_3_exp, "results/HU_outside_model_3_exp.csv")
#===============
#Classification rate
...
model3_variance <- 1.644^2; model3_variance 
model3_vpc <-   1.644^2 / (1.644^2 +3.29)*100; model3_vpc

# 45.01%

#PCV
model1_variance <-  3.025^2
model3_variance <-  1.644^2
model3_PCV <- (3.025^2 - 1.644^2)/ 3.025^2 ; model3_PCV
model3_PCV <- model3_PCV*100; model3_PCV
#PCV=70.46%
#This indicates 29.54% of variance was not explained by adding strata and covariates 
#=================================================================================================
#Plot
...
plot3 <- r3_Intercept2 %>%
  ...
  geom_errorbar(aes(ymin=Q2.5, ymax=Q97.5)) +
  scale_x_continuous(breaks = round(seq(min(r2_Intercept2$Strata_ID), max(r2_Intercept2$Strata_ID), by = 5),1)) +
  scale_y_continuous(breaks = round(seq(-12,12, by = 2),1))+
...
#Export data
write.csv(HU_outside_model_model3, file = "results/HU_outside_model3.csv")
...
ggsave('results/HU_outside_model_plot.pdf',HU_outside_model_plot, width =18,height=15,dpi=1200)
#ggsave('results/HU_outside_model_plot.tiff',HU_outside_model_plot, width =18,height=15,dpi=1200)
ggsave('results/HU_outside_model_plot.eps',HU_outside_model_plot, width =18,height=15,dpi=1200)
#===============================================================================
